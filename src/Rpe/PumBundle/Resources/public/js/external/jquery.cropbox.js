    ...
    Crop.prototype = {
      init: function () {
        ...
        var defaultControls = $('<div/>', { 'class' : 'cropControls' })
              .append($('<span>Cliquez pour cadrer</span>'))
      ...
      updateOptions: function () {
        ...
        if (self.options.showControls === 'always' || self.options.showControls === 'auto' && is_touch_device())
          ...
          self.$frame.on('mouseenter.' + pluginName, function () { self.$frame.removeClass('hover'); });
          self.$frame.on('mouseleave.' + pluginName, function () { self.$frame.addClass('hover'); });
        }
